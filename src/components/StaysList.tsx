...
export function StaysList() {
  ...
  const staysQuery = useMemoFirebase(() => {
    ...
    return query(collection(firestore, "accommodations"), orderBy("createdAt", "desc"));
  }, [firestore]);
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
      {allStays.map((hotel, idx) => {
        const img = PlaceHolderImages.find(p => p.id === (hotel as any).imageId || p.id === "hotel-niyibituronsa");
        return (
          <Card key={hotel.id || idx} className="group border-none shadow-xl hover:shadow-2xl transition-all duration-500 rounded-3xl overflow-hidden bg-white">
            ...
            <CardContent className="p-6">
              <div className="flex items-center gap-1 text-muted-foreground text-xs font-bold uppercase tracking-widest mb-2">
                ...
                {hotel.address || (hotel as any).location || "Rumonge"}
              </div>
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {((hotel as any).amenities || ["Wifi", "Lakeside"]).map((amenity: string, aIdx: number) => (
              ...
              <div className="flex items-center justify-between border-t pt-4">
                <div className="flex items-center gap-1">
                  ...
                  <span className="font-bold text-sm">{(hotel as any).rating || "4.5"}</span>
                </div>
                <Link 
                  href={`/stays/${hotel.id}`}
